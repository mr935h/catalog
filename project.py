...
def showItem(category, category_item):
    ...
    if cat:
        item = session.query(Categories).filter_by(
                category=category, item=category_item).one()
...
@app.route('''/catalog/<string:category>/
            <string:category_item>/edit/''', methods=('GET', 'POST'))
def editCatalogItem(category, category_item):
    ...
    if request.method == 'POST':
        ...
        return render_template(
                'edit_item.html', category=category, category_item=editedItem)


# delete item from catalog
@app.route('''/catalog/<string:category>/
            <string:category_item>/delete/''', methods=('GET', 'POST'))
def deleteCatalogItem(category, category_item):
    ...
    category = session.query(Categories).filter_by(
            category=category, item=category_item).one()
    user = session.query(Author).join(Categories).filter_by(
            item=deleteItem.item).one()
    if user.user_name != login_session['email']:
        return """<script>function myFunction()
            {alert('You are not authorized to delete this item.
            Please create your own listing in order to delete.');}</script>
            <body onload='myFunction()''>"""
    if request.method == 'POST':
...
def newItem():
    ...
    if request.method == 'POST':
        ...
        if user == '':
            newItemuser = Author(
                    name='Michael Ryden',
                    user_name=login_session['email'])
            session.add(newItemuser)
            session.commit()
            user = session.query(Author).filter_by(
                    user_name=login_session['email']).one()
        newItem = Categories(
                category=request.form['category'],
                item=request.form['item'],
                item_description=request.form['item_description'],
                image=request.form['item_url'],
                author_id=user.id)
        session.add(newItem)
